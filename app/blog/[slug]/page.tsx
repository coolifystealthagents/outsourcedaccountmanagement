...
function CommunicationChart({ points }: { points: Array<{ label: string; value: number; color: string }> }) {
  return <figure className="article-visual" data-visual="communication-chart">
    <svg viewBox="0 0 760 330" role="img" aria-labelledby="communication-chart-title communication-chart-desc">
      ...
      <text x="30" y="42" className="svg-title">Where the workday went</text>
      {points.map((point, index) => {
        ...
        return <g key={point.label}>
          ...
          <rect x="190" y={y} width={point.value * 10} height="30" rx="6" fill={point.color} />
          <text x={Math.min(680, 205 + point.value * 10)} y={y + 21} className="svg-value">{point.value}%</text>
        </g>;
      })}
    </svg>
    <figcaption><strong>Method note:</strong> Microsoft reported the split from aggregated Microsoft 365 activity over 180 days. Meetings, chat, and email add to 57% communication time; file creation adds to 43%. The report was published May 9, 2023.</figcaption>
  </figure>;
}
...
function HandoffGraphic() {
  return <figure className="article-visual" data-visual="handoff-ownership-graphic">
    <svg viewBox="0 0 760 300" role="img" aria-labelledby="handoff-graphic-title handoff-graphic-desc">
      ...
      <text x="30" y="42" className="svg-title">A clean handoff path</text>
      <g transform="translate(30 78)">
        <rect width="190" height="150" rx="16" fill="#e8c98e" />
        <text x="20" y="38" className="svg-step">1  Capture</text>
        <text x="20" y="70" className="svg-copy">History, promises,</text>
        <text x="20" y="94" className="svg-copy">risks, next action</text>
      </g>
      <path d="M230 153 H278" stroke="#132a2f" strokeWidth="4" markerEnd="url(#arrow)" />
      <g transform="translate(285 78)">
        <rect width="190" height="150" rx="16" fill="#b9dfce" />
        <text x="20" y="38" className="svg-step">2  Check</text>
        <text x="20" y="70" className="svg-copy">Owner, access,</text>
        <text x="20" y="94" className="svg-copy">approval limits</text>
      </g>
      <path d="M485 153 H533" stroke="#132a2f" strokeWidth="4" markerEnd="url(#arrow)" />
      <g transform="translate(540 78)">
        <rect width="190" height="150" rx="16" fill="#ffd9ce" />
        <text x="20" y="38" className="svg-step">3  Review</text>
        <text x="20" y="70" className="svg-copy">First notes, client</text>
        <text x="20" y="94" className="svg-copy">message, risk view</text>
      </g>
    ...
    <figcaption>The handoff record sits between the old owner and the new manager. The internal account owner checks the record and the first live work before the account list grows.</figcaption>
  </figure>;
}
  ...
  return <><Header /><main className="article-page">
    ...
    <article className="container rich-article" data-article-slug={slug}>
      ...
      <div className="article-body">
        ...
        <aside className="takeaway-box"><h2>What to carry into the handoff</h2><ul>{rich.takeaways.map((item) => <li key={item}>{item}</li>)}</ul></aside>
        ...
        <section className="client-script">
          <h2>A short client handoff note</h2>
          <p>Use this as a starting point, then replace the bracketed date and match the wording to the client relationship. Send it only after the account owner checks the handoff map.</p>
          <div className="script-copy">{rich.script.map((line) => <p key={line}>{line}</p>)}</div>
        </section>

        <section className="article-faq">
          <h2>Questions about the handoff</h2>
